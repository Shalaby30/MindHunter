...
export default function WatchTVError({ error, reset }) {
  useEffect(() => {
    console.error(error);
  }, [error]);
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-4 pt-24 text-center">
      ...
      <h1 className="mb-3 text-2xl font-bold text-foreground">Player error</h1>
      <p className="mb-8 max-w-md text-sm text-muted-foreground">
        Something went wrong with the player. Please try again.
      </p>
      <button
        onClick={reset}
